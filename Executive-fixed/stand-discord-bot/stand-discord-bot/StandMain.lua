...
-- CAMLOCK: every frame camera + mouse stick to target head
...
Connections.Camlock = RunService.RenderStepped:Connect(camlockStep)
...
local function reloadGun(gun)
    ...
    -- 4) Unequip / re-equip cycle forces ammo refresh on some places
    pcall(function()
        local h = getHum()
        ...
        if h and bag then
            h:UnequipTools()
            task.wait(0.08)
            if gun.Parent == bag or gun.Parent == getChar() then
                h:EquipTool(gun)
            else
                local again = findToolByName(PreferredGun, false)
                if again then h:EquipTool(again) end
            end
        end
    end)
...
local function lockOnTarget(plr, sideDist, height)
    ...
    local pos = their.Position - look * sideDist + right * 0.1 + Vector3.new(0, height, 0)
    pcall(function()
...
local function punch(plr)
    ...
    setCamlock(plr, 0.6)
    if not lockOnTarget(plr, 1.8, 0.4) then return end
    aimAt(plr)
...
local function shoot(plr)
    ...
    -- CAMLOCK onto this player while we shoot
    setCamlock(plr, 1.2)
    ...
    -- stay glued to target + camlock fires mouse every frame
    for i = 1, 8 do
        if not isAlive(plr) then break end
        lockOnTarget(plr, 1.8 + (i % 3) * 0.12, 0.9)
        local aim = aimAt(plr)
        if aim then
            fireMouse(aim)
            fireMouse(aim)
        ...
        if MainEvent then
            ...
            if aim then pcall(function() MainEvent:FireServer("Shoot", aim) end) end
        end
        task.wait(0.02)
    end

    -- always try reload after a burst (ammo values unreliable on some places)
    task.spawn(function()
        task.wait(0.08)
        local g = findToolByName(PreferredGun, false)
        if g then
