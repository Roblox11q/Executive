...
-- Throttle body teleports so server/other clients see the alt (spam CFrame = desync/invisible)
local lastBodyLock = 0
local BODY_LOCK_INTERVAL = 0.12
...
local function ensureVisible()
    ...
    pcall(function()
        ...
        local h = getHum()
        if h then
            h.Health = math.max(h.Health, 1)
        end
    end)
end

local function lockOnTarget(plr, behindDist, height)
    -- BEHIND + slightly above, face HEAD — body moves throttled so it replicates
    ...
    -- never attack from void (causes invisible desync)
    ...
    local now = tick()
    -- only teleport body every BODY_LOCK_INTERVAL so other clients see it
    if (now - lastBodyLock) >= BODY_LOCK_INTERVAL then
        ...
        pcall(function()
            ...
            -- small velocity helps physics replicate instead of sleeping
            my.AssemblyLinearVelocity = (pos - my.Position) * 2
            my.AssemblyAngularVelocity = Vector3.zero
        end)
    else
        -- between teleports, still face target without full teleport spam
        pcall(function()
            my.CFrame = CFrame.new(my.Position, lookAt)
        end)
    end
    return true
end
...
local function softReload(gun)
    -- reload in place — never void (void was breaking aim)
