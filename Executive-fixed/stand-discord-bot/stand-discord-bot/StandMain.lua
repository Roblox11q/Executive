...
local function lockOnTarget(plr, behindDist, height)
    -- stand BEHIND target a few studs (not inside body), facing their head
    behindDist = behindDist or 3.5
    height = height or 1.0
    local my = getHRP()
    local their = getHRP(plr)
    if not my or not their then return false end
    local head = getAimPart(plr)
    local lookAt = (head and head.Position) or (their.Position + Vector3.new(0, 1.4, 0))
...
local function punch(plr)
    ...
    setCamlock(plr, 0.8)
    if not lockOnTarget(plr, 2.8, 0.5) then return end
    aimAt(plr)
...
local function shoot(plr)
    ...
    for i = 1, 10 do
        ...
        -- stay ~3–4 studs behind, slight variation so not glued
        lockOnTarget(plr, 3.2 + (i % 3) * 0.25, 1.0)
        local aim = aimAt(plr)
        if aim then
            fireMouse(aim)
            fireMouse(aim)
...
local function cmdKnock(user)
    ...
    task.spawn(function()
        ...
        State.Tracking = false
        setCamlock(plr, 8)
        -- pre-aim behind target so FIRST shot faces them (not inside body)
        lockOnTarget(plr, 3.5, 1.0)
        aimAt(plr)
        task.wait(0.05)
        for i = 1, 45 do
