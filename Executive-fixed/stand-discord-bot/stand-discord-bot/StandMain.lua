...
-- Body position: SNAP once then walk — spam CFrame = invisible on main / flying ragdoll
local lastBodyLock = 0
local BODY_LOCK_INTERVAL = 0.55
...
local function lockOnTarget(plr, behindDist, height)
    ...
    -- stay on ground-ish (height 1.5) behind them — no flying pose
    local pos = their.Position + back * behindDist + Vector3.new(0, height, 0)
    local distNow = (my.Position - pos).Magnitude

    local now = tick()
    -- only hard-teleport if far away; otherwise MoveTo so server sees walking
    if distNow > 14 or (now - lastBodyLock) >= BODY_LOCK_INTERVAL then
        lastBodyLock = now
        ...
        -- soft face + walk toward lock point (replicates better)
        pcall(function()
            local h = getHum()
            if h then
                h:MoveTo(pos)
