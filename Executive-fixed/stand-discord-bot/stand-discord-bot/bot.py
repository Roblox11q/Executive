...
def generate_loader(cfg: dict) -> str:
    ...
    if main_src and len(main_src) > 50:
        a("    if not src then")
        a("        print(\"[Stand] GitHub main not set/failed — using embedded main\")")
        a("        src = [=[")
        a(main_src)
        a("]=]")
        a("    else")
        a("        print(\"[Stand] Main downloaded from GitHub\")")
        a("    end")
    else:
        a("    if not src then")
        a("        error(\"Upload StandMain.lua to GitHub and set MAIN_SCRIPT_URL to the raw URL\")")
        a("    end")
        a("    print(\"[Stand] Main downloaded from GitHub\")")
