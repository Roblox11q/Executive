...
local function lockOnTarget(plr, sideDist, height)
    sideDist = sideDist or 2.0
    height = height or 0.8
    local my = getHRP()
    local their = getHRP(plr)
    local head = getAimPart(plr)
    if not my or not their then return false end
    -- always approach from OUR current direction toward them, facing their head
    -- (fixes first-shot "behind target looking away")
    local lookAt = (head and head.Position) or (their.Position + Vector3.new(0, 1.5, 0))
    local from = my.Position
    local dir = lookAt - from
    if dir.Magnitude < 0.1 then dir = their.CFrame.LookVector else dir = dir.Unit end
    local pos = lookAt - dir * sideDist + Vector3.new(0, height * 0.15, 0)
    pcall(function()
...
local function shoot(plr)
    ...
    -- soft reload — void while reloading so kill-spawns don't hit the alt
    ...
    -- first frame: hard face target so first shot hits
    lockOnTarget(plr, 2.2, 0.9)
    aimAt(plr)
    task.wait(0.02)
    ...
    for i = 1, 10 do
        ...
        lockOnTarget(plr, 1.6 + (i % 3) * 0.1, 0.8)
        local aim = aimAt(plr)
        if aim then
            fireMouse(aim)
            fireMouse(aim)
...
local function followOwner()
    if IsOwner then return end
    if State.InVoid or State.Reloading then
        local hrp = getHRP()
        if hrp then
            pcall(function()
                hrp.CFrame = VoidCF
...
local function cmdKnock(user)
    ...
    task.spawn(function()
        ...
        State.Tracking = false
        setCamlock(plr, 8)
        -- pre-aim so FIRST shot faces them
        lockOnTarget(plr, 2.2, 0.9)
        aimAt(plr)
        task.wait(0.05)
        for i = 1, 45 do
...
Connections.Main = RunService.Heartbeat:Connect(function()
    ...
    if State.LoopKill and not isKO(LocalPlayer) and not State.Reloading then
    ...
    if ka and not State.LoopKill and not isKO(LocalPlayer) and not State.Reloading and tick() - lastKA > 0.08 then
        lastKA = tick()
