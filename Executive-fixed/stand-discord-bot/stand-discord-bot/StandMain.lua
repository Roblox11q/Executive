...
local function lockOnTarget(plr, behindDist, height)
    -- ABOVE + BEHIND target, camera/character look at HEAD (never inside body)
    behindDist = behindDist or 5.5
    ...
    -- hard minimum distance so we never clip into them
    if (pos - their.Position).Magnitude < 5 then
        pos = their.Position + back * 5.5 + Vector3.new(0, 4, 0)
    end
...
local function shoot(plr)
    ...
    for i = 1, 10 do
        ...
        lockOnTarget(plr, 5.0 + (i % 3) * 0.3, 3.8 + (i % 2) * 0.2)
        local aim = aimAt(plr)
        if aim then
            fireMouse(aim)
            fireMouse(aim)
