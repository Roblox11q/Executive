...
def generate_loader(cfg: dict) -> str:
    ...
    a("local function fetchMain()")
    a("    if type(MAIN_URL) ~= \"string\" or MAIN_URL == \"\" or string.find(MAIN_URL, \"YOUR_USER\", 1, true) then")
    a("        return nil")
    a("    end")
    a("    local body")
    a("    pcall(function() body = game:HttpGet(MAIN_URL) end)")
    a("    if type(body) == \"string\" and #body > 50 and not string.find(body, \"404: Not Found\", 1, true) then")
    a("        return body")
    a("    end")
    a("    local res = httpRequest({ Url = MAIN_URL, Method = \"GET\" })")
    a("    if res then")
    a("        body = res.Body or res.body")
    a("        if type(body) == \"string\" and #body > 50 then return body end")
    a("    end")
    a("    return nil")
    a("end")
    a("")
    a("local function loadMain()")
    a("    local src = fetchMain()")
    if main_src and len(main_src) > 50:
        a('    if not src then')
        a('        print("[Stand] Hosted main failed - using embedded main")')
        a('        src = [=[')
        a(main_src)
        a(']=]')
        a('    else')
        a('        print("[Stand] Main downloaded from host")')
        a('    end')
    else:
        a('    if not src then')
        a('        error("Upload StandMain.lua / set MAIN_SCRIPT_URL or product host plain mode")')
        a('    end')
        a('    print("[Stand] Main downloaded from host")')
    a('    if type(src) ~= "string" or #src < 50 then')
    a('        error("main source empty or too short: " .. tostring(src and #src))')
    a('    end')
    a('    -- Re-bind config into every common executor env before compile/run')
    a('    pcall(function()')
    a('        _G.StandConfig = StandConfig')
    a('        if getgenv then getgenv().StandConfig = StandConfig end')
    a('        if shared then shared.StandConfig = StandConfig end')
    a('    end)')
    a('    -- Bootstrap so main always sees Config even if envs differ')
    a('    local boot = "do\\n"')
    ...
    a('        .. "end\\nend\\n"')
    a('    src = boot .. src')
    a('    print("[Stand] Compiling main | bytes:", #src)')
    ...
    a('    fn()')
    a('    print("[Stand] Main chunk returned (if no Main loaded line, Config missing or early return)")')
    a('end')
    a('')
