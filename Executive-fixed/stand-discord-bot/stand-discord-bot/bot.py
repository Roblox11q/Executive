...
def generate_loader(cfg: dict) -> str:
    ...
    a("")
    a("-- Main script lives on GitHub (short loader = config only)")
    ...
    a("    -- 2) executor request")
    a("    local res = httpRequest({ Url = MAIN_URL, Method = \"GET\" })")
    a("    if res then")
    a("        body = res.Body or res.body")
    a("        if type(body) == \"string\" and #body > 50 then return body end")
    a("    end")
    a("    error(\"Could not download main script. Set MAIN_SCRIPT_URL to raw GitHub link.\")")
    a("end")
    a("")
