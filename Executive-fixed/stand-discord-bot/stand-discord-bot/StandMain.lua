...
-- Same style as the camlock script: CFrame.lookAt head every RenderStepped
local function camlockStep()
    ...
    if their then
        local vel = their.AssemblyLinearVelocity or Vector3.zero
        aim = aim + Vector3.new(vel.X, math.clamp(vel.Y, -12, 12) * 0.2, vel.Z) * 0.15
    end
    Camera = Workspace.CurrentCamera
    if Camera then
        -- direct lock like the reference camlock script
        pcall(function()
